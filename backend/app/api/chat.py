...
def search_repository(request: Request, body: SearchRequest):
    try:
        results = search_code(body.query, repo_name=body.repo_name, language_filter=body.language)
...
def chat_with_repo(request: Request, body: ChatRequest):
    try:
        ...
        answer_string, rerank_scores = generate_explanation(
            body.repo_name, 
            body.question, 
            history,
...
# Tour endpoint logic remains the same (uses build_repo_graph which we updated)
import os
import json
...
def generate_tour(request: TourRequest):
    try:
        repo_path = os.path.join(TEMP_REPO_DIR, request.repo_name)
...
Format exactly as a JSON array of objects: [{{ "step": 1, "title": "...", "explanation": "...", "key_file": "..." }}]."""
